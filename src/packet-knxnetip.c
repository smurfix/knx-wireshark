...
static void dissect_knxnetip (tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree) {
    ...
    /* HEADER*/
    proto_item *ti = proto_tree_add_text(knx_tree, tvb, offset, 6, "Header");
    header_tree = proto_item_add_subtree(ti, ett_knxnetip_header);
    ...
    offset+=2;
    /* BODY */
    ti = proto_tree_add_text(knx_tree, tvb, offset, -1, "Body");
    body_tree = proto_item_add_subtree(ti, ett_knxnetip_body);
